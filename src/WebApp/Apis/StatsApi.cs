...
public static class StatsApi
{
    ...
    private static async Task<Ok<IReadOnlyCollection<string>>>
        GetPlayerHeroesAsync([AsParameters] StatsServices services, HttpContext context, int playerId,
            CancellationToken ct = default)
    {
        var result = await services.StatsHubService.GetPlayerHeroesAsync(playerId, ct);
